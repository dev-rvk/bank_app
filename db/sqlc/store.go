...
// this store should have all the function of queries struct and one transfer function
type Store interface{
	TransferTx (ctx context.Context, args TransferTxParams) (TransferTxResults, error)
	Querier
}
// store struct (SQLStore for real database)
...
func (store *SQLStore) TransferTx (ctx context.Context, args TransferTxParams) (TransferTxResults, error) {
	...
	err :=  store.execTx(ctx, func(q *Queries) error {
		...
		// add condition to update the account with smaller account id first to avoid deadlock
